...
//! * If it exists, we recursively search for `jvm` library file inside
//!   `JAVA_HOME` directory.
//! * If it is not set, we use the following commmand to find actual JVM home
//!   directory: ```bash java -XshowSettings:properties -version | grep
//!   'java.home' ``` Then, we search for `jvm` as we have `JAVA_HOME`.
//!
...
fn main() {
    println!("customized build to load jni library");
